...
import getSecuritySign from './sign.js' // 获取签名方法

const ERR_OK = 0
const token = 5381

const fallbackPicUrl =
  'https://y.gtimg.cn/mediastyle/music_v11/extra/default_300x300.jpg?max_age=31536000' // 歌曲图片加载失败时使用的默认图片
...
function registerSongsUrl(app) {
  app.use('/api/getSongsUrl', (req, res) => {
    const { mid } = getUrlParams(req.url)
...
function registerTopDetail(app) {
  app.use('/api/getTopDetail', (req, res) => {
    ...
    const { id, period } = getUrlParams(req.url)
...
function registerHotKeys(app) {
  app.use('/api/getHotKeys', (req, res) => {
...
function registerSearch(app) {
  app.use('/api/search', (req, res) => {
    ...
    const { query, page, showSinger } = getUrlParams(req.url)
